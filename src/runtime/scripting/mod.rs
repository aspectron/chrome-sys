...
extern "C" {
    ...
    pub fn set_match_origin_as_fallback(this: &RegisteredContentScript, match_origin_as_fallback:bool);
...
pub fn vector_to_array(values: Vec<&str>)->Array{
    Array::from_iter(values.iter().map(|a|JsValue::from(*a)).collect::<Vec<JsValue>>())
}
...
pub fn array_to_vector(values: Array)->Vec<String>{
    values.into_iter().map(|a|a.as_string().unwrap_or_default()).collect()
}
